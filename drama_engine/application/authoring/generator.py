...
SCRIPT_DIR = Path(__file__).resolve().parents[2] / "core" / "scripts"
...
def build_default_authoring_templates() -> list[AuthoringTemplate]:
    ...
    return [
        AuthoringTemplate(
            ...
            script_name="werewolf_v1_guard.yaml",
            runtime_type="game_session",
            extensions=(),
            rule_set="script.effects",
            ...
            defaults={"players": 12, "runtime": "game_session"},
        ),
        AuthoringTemplate(
            game_type="mission_vote",
            script_name="avalon.yaml",
            runtime_type="game_session",
            extensions=("avalon",),
            rule_set="avalon.effects",
            keywords=("阿瓦隆", "任务", "组队", "刺杀", "mission", "avalon"),
            required_questions=("任务人数表是什么？", "有哪些阵营身份？", "几次任务成功/失败结束？"),
            defaults={"players": 5, "runtime": "game_session"},
        ),
        AuthoringTemplate(
            game_type="word_guess",
            script_name="who_is_undercover.yaml",
            runtime_type="game_session",
            extensions=(),
            rule_set="builtin.party.free_discussion",
            keywords=("谁是卧底", "词语", "猜词", "卧底", "word", "guess"),
            required_questions=("平民词和卧底词如何生成？", "每轮发言几次？", "卧底是否有猜词机会？"),
            defaults={"players": 6, "runtime": "game_session"},
        ),
        AuthoringTemplate(
            game_type="card_game",
            script_name="uno_lite.yaml",
            runtime_type="game_session",
            extensions=("cards",),
            rule_set="builtin.cards.uno_lite",
            keywords=("卡牌", "UNO", "uno", "出牌", "摸牌", "deck", "card"),
            required_questions=("有哪些牌？", "初始手牌几张？", "摸牌和出牌规则是什么？", "胜利条件是什么？"),
            defaults={"players": 4, "runtime": "game_session"},
        ),
        AuthoringTemplate(
            game_type="board_game",
            script_name="gomoku_lite.yaml",
            runtime_type="game_session",
            extensions=("board",),
            rule_set="builtin.board.gomoku_lite",
            keywords=("棋盘", "五子棋", "象棋", "围棋", "跳棋", "落子", "board"),
            required_questions=("棋盘尺寸是多少？", "玩家如何移动或落子？", "合法动作和胜利条件是什么？"),
            defaults={"players": 2, "runtime": "game_session"},
        ),
        AuthoringTemplate(
            game_type="map_economy",
            script_name="monopoly_lite.yaml",
            runtime_type="game_session",
            extensions=("board", "dice", "economy"),
            rule_set="builtin.board.monopoly_lite",
        ...
        AuthoringTemplate(
            game_type="ttrpg",
            script_name="dynamic_story_runtime_lite.yaml",
            runtime_type="dynamic_story",
            extensions=("story", "dice"),
            rule_set="dynamic_story.runtime",
            keywords=("DND", "dnd", "COC", "跑团", "剧情", "冒险", "DM", "ttrpg"),
            required_questions=("世界设定是什么？", "角色卡包含哪些属性？", "检定骰和失败后果是什么？"),
            defaults={"players": 3, "runtime": "dynamic_story"},
            risk_warnings=("完整自由行动需要 DynamicStoryInterpreter 和 DM policy。",),
        ),
        AuthoringTemplate(
            game_type="group_chat",
            script_name="group_chat_room_lite.yaml",
            runtime_type="group_chat",
            extensions=(),
            rule_set="group_chat.runtime",
            keywords=("群聊", "多 Agent", "多agent", "圆桌", "讨论", "group_chat", "chat"),
            required_questions=("群聊主题是什么？", "有哪些参与者角色？", "最多讨论几轮？"),
            defaults={"players": 3, "runtime": "group_chat"},
            risk_warnings=("完整自由发言需要 GroupChatInterpreter 和 TranscriptMemory。",),
        ),
    ]
