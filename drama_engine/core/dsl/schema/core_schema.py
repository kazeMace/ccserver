...
ROLE_FIELD_SCHEMA = DslSchema(
    ...
    fields=(
        ...
        DslField("brief", "string", False, "简短身份说明。"),
        DslField("description", "string", False, "详细角色人设，用于 AI prompt 注入。"),
        DslField("portrait_url", "string", False, "角色头像图片 URL。"),
        DslField("emoji", "string", False, "备用 emoji 头像。"),
        DslField("voice_id", "string", False, "TTS 音色 ID。"),
        DslField("tts_config", "object", False, "TTS 配置对象（provider/speed/pitch 等）。"),
...
CONDITION_FIELD_SCHEMA = DslSchema(
    ...
    fields=(
        ...
        DslField("id", "string", False, "语义化 executor ID，例如 story_ending_judge。"),
        DslField("runtime", "string", False, "code executor 运行环境，例如 python、shell、node、bun。"),
        DslField("env", "object", False, "code executor 环境变量。"),
        DslField("code", "string", False, "code executor 代码；condition 只能返回布尔结果。"),
        DslField("endpoint", "string", False, "http/llm executor 的语义端点名，由后台映射真实 URL。"),
        DslField("input", "object", False, "http/llm executor 输入，可包含 {ref: ...}。"),
        DslField("pass_when", "condition", False, "http/llm executor 返回结构后的二次判断条件。"),
