"""
model_info_registry — 全局模型能力注册表。

进程级单例，线程安全。首次访问时加载内置模型目录（BUILTIN_MODEL_CATALOG）。

提供按 model_id 查询、按 provider 过滤、按 input_type 筛选等能力。
Agent 等消费者通过此注册表判断模型的输入能力。
"""
...
class ModelInfoRegistry:
    ...
    def list_by_provider(self, provider: str) -> list[ModelInfo]:
        ...
        Args:
            provider: provider id，如 "anthropic"、"openai"
