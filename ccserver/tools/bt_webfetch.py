...
Flow:
  ...
  4. Call Claude Haiku with the user's prompt to extract relevant information.
    ...
    async def run(self, url: str, prompt: str) -> ToolResult:
        try:
            ...
            if not (url.startswith("http://") or url.startswith("https://")):
                return ToolResult.error(f"Invalid URL: must start with http:// or https://")
            ...
            # Step 3: Use Haiku to process the content with the user's prompt.
            result = await _apply_prompt(self.client, url, markdown_content, prompt)
...
async def _apply_prompt(client: AsyncAnthropic, url: str, content: str, prompt: str) -> str:
    """
    Call Claude Haiku to answer the prompt based on the fetched page content.
