...
try:
    ...
    raise ImportError(
        "PyYAML is required for HOOK.md parsing. "
...
class HookLoader:
    ...
    def _parse_hook_md(self, hook_md: Path) -> Optional[dict]:
        ...
        if not stripped.startswith("---"):
            logger.warning("HOOK.md has no YAML frontmatter | path={}", hook_md)
            return None

        # 找第二个 ---
        end_index = stripped.find("\n---", 3)
        if end_index == -1:
            logger.warning("HOOK.md frontmatter not closed (missing closing ---) | path={}", hook_md)
            return None

        yaml_text = stripped[4:end_index]  # 跳过开头的 ---\n

        try:
            data = yaml.safe_load(yaml_text)
        except Exception as e:
            logger.warning("HOOK.md YAML parse failed | path={} error={}", hook_md, e)
            return None
    ...
    async def _run_bun(self, entry: HookEntry, payload: dict, ctx: HookContext) -> HookResult:
        ...
        # bun_wrapper.ts 位于 ccserver/hooks/ 目录下
