"""
tests/test_frontmatter.py — parse_frontmatter 单元测试
...
from ccserver.utils.frontmatter import parse
...
def test_inline_list_with_spaces_around_commas():
    text = "---\ntags: a, b, c\n---\n"
    meta, _ = parse(text)
    assert meta["tags"] == ["a", "b", "c"]


def test_inline_list_two_items():
    text = "---\ncolors: red,blue\n---\n"
    meta, _ = parse(text)
...
def test_multiline_string_strips_trailing_newline():
    text = "---\ntext: |\n  hello\n  world\n---\n"
    meta, _ = parse(text)
