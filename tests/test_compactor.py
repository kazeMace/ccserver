...
覆盖：
  - should_compact() 阈值判断（通过 TriggerPolicy）
  - run_micro() 截断旧工具结果（in-place 修改）
  - run_micro() 保留最近 keep_recent 个完整结果
  - run_micro() 在结果数 <= keep_recent 时不截断
  - run_micro() 工具名通过 tool_name_map 正确注入
  - run_micro() 只截断长内容（>100 字符），短内容保留
"""

from unittest.mock import MagicMock

from ccserver.compact import Compactor, CompactorFactory
from ccserver.compact.micro import DefaultMicroCompactor
from ccserver.compact.trigger import DefaultTriggerPolicy


def _make_compactor(threshold=1000, keep_recent=2) -> Compactor:
    """用指定 threshold 和 keep_recent 构建 Compactor（使用 Factory）。"""
    return CompactorFactory.build_default(
        adapter=MagicMock(),
        trigger=DefaultTriggerPolicy(threshold=threshold),
        micro=DefaultMicroCompactor(keep_recent=keep_recent),
    )
...
def test_needs_compact_below_threshold():
    ...
    should, _ = comp.should_compact(messages)
...
def test_micro_keeps_recent_n_intact():
    ...
    long_content = "y" * 200
    messages = _make_messages(
        ("Bash", long_content),   # 截断
        ("Read", long_content),   # 保留
        ("Grep", long_content),   # 保留
    )
    comp.run_micro(messages)
    results = [
        part
...
def test_micro_returns_same_messages_list():
    ...
    returned = comp.run_micro(messages)
    # run_micro() 应返回同一个列表对象（in-place 修改）
    assert returned is messages


def test_micro_handles_empty_messages():
    comp = _make_compactor()
    result = comp.run_micro([])
    assert result == []
