...
初始化（以下三项并行执行）：
1. 读取文件 `.ccserver/skills/query-rewrite/SKILL.md`，加载 Query 改写规范，记为 **QUERY_REWRITE_SKILL**。Step 2 严格按照其定义的流程执行。从上下文 `[当前时间]` 字段提取当前日期，作为 `date` 参数传入，**严禁使用训练知识中的年份**。
2. 读取 `data/current_persona.md`，提取角色城市信息，记为 **PERSONA**。
3. 调用 `get_user_profile()` 获取用户兴趣偏好。
