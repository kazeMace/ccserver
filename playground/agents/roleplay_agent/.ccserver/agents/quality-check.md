...
- `history`：最近 3-5 轮对话（用于重复检测和话题判断）
...
查看 history 中最近 3 轮 user 回复：
当assistant提到某个话题，用户明确表示不感兴趣或者在追问下，用户不予理睬，判定用户不感兴趣，需要转换话题
...
观察 history 中最近 5 轮 assistant 回复的平均字数：若均低于 25 字，判定为话题停滞（对话陷入无实质内容的来回）。
...
- `issues`：问题列表，每项引用回复中具体片段（如 `"第2句出现OOC：'作为AI助手我无法…'"`)
- `reflection`：具体修复指引——指出需要去掉哪些内容、保留哪些内容、应以什么口吻重写。`passed=true` 时为空字符串。

## 判断原则

- **OOC 或拒绝回答** → `passed=false`，`severity=high`
- **明显重复** → `passed=false`，`severity=medium`
- **填充词严重或话题停滞** → `passed=true`，`severity=low`（记录问题但不触发重试）
